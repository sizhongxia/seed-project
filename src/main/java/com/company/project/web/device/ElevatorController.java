...
import com.company.project.model.om.DeviceElevatorModel;
...
@RestController
@RequestMapping("/apiv0/device/elevator")
public class ElevatorController {
	private Logger logger = LoggerFactory.getLogger(ElevatorController.class.getName());
	...
	@Resource
	private EquipmentLiftParameterService equipmentLiftParameterService;

	@TokenCheck
	@PostMapping("/getById")
	public Result<?> getById(@RequestBody DeviceElevatorModel model) {
		...
		DeviceElevatorModel res = new DeviceElevatorModel();
	...
	public Result<?> deleteById(@RequestBody DeviceElevatorModel model) {
		if (StringUtils.isBlank(model.getUuid())) {
	...
	@TokenCheck
	@PostMapping("/save")
	public Result<?> save(@Validated @RequestBody DeviceElevatorModel model, BindingResult bindingResult,
		...
		EquipmentLiftParameter equipmentLiftParameter = new EquipmentLiftParameter();
		equipmentLiftParameter.setUuid(equipmentBasics.getUuid());
		equipmentLiftParameter.setEquipmentno(equipmentBasics.getEquipmentno());
		equipmentLiftParameter.setCreatTime(System.currentTimeMillis());
		equipmentLiftParameterService.save(equipmentLiftParameter);
	...
	@TokenCheck
	@PostMapping("/update")
	public Result<?> update(@Validated @RequestBody DeviceElevatorModel model, BindingResult bindingResult,
			HttpServletRequest request) {
		if (bindingResult.hasErrors()) {
			return ResultGenerator.genFailResult(bindingResult.getFieldError().getDefaultMessage());
		}
		...
		EquipmentLiftParameter equipmentLiftParameter = equipmentLiftParameterService.findBy("uuid",
				equipmentBasics.getUuid());
		if (equipmentLiftParameter == null) {
			equipmentLiftParameter = new EquipmentLiftParameter();
			equipmentLiftParameter.setUuid(equipmentBasics.getUuid());
			equipmentLiftParameter.setEquipmentno(equipmentBasics.getEquipmentno());
			equipmentLiftParameter.setCreatTime(System.currentTimeMillis());
			equipmentLiftParameterService.save(equipmentLiftParameter);
