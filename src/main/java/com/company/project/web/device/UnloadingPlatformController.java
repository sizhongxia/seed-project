...
import com.company.project.model.om.DeviceUnloadingPlatformModel;
...
public class UnloadingPlatformController {
	...
	@Resource
	private EquipmentDischargeParameterService equipmentDischargeParameterService;

	@TokenCheck
	@PostMapping("/getById")
	public Result<?> getById(@RequestBody DeviceUnloadingPlatformModel model) {
		...
		DeviceUnloadingPlatformModel res = new DeviceUnloadingPlatformModel();
	...
	public Result<?> deleteById(@RequestBody DeviceUnloadingPlatformModel model) {
		if (StringUtils.isBlank(model.getUuid())) {
	...
	@TokenCheck
	@PostMapping("/save")
	public Result<?> save(@Validated @RequestBody DeviceUnloadingPlatformModel model, BindingResult bindingResult,
		...
		EquipmentDischargeParameter equipmentDischargeParameter = new EquipmentDischargeParameter();
		equipmentDischargeParameter.setUuid(equipmentBasics.getUuid());
		equipmentDischargeParameter.setEquipmentno(equipmentBasics.getEquipmentno());
		equipmentDischargeParameter.setCreatTime(System.currentTimeMillis());
		equipmentDischargeParameterService.save(equipmentDischargeParameter);
	...
	@TokenCheck
	@PostMapping("/update")
	public Result<?> update(@Validated @RequestBody DeviceUnloadingPlatformModel model, BindingResult bindingResult,
			HttpServletRequest request) {
		if (bindingResult.hasErrors()) {
			return ResultGenerator.genFailResult(bindingResult.getFieldError().getDefaultMessage());
		}
		...
		EquipmentDischargeParameter equipmentDischargeParameter = equipmentDischargeParameterService.findBy("uuid",
				equipmentBasics.getUuid());
		if (equipmentDischargeParameter == null) {
			equipmentDischargeParameter = new EquipmentDischargeParameter();
			equipmentDischargeParameter.setUuid(equipmentBasics.getUuid());
			equipmentDischargeParameter.setEquipmentno(equipmentBasics.getEquipmentno());
			equipmentDischargeParameter.setCreatTime(System.currentTimeMillis());
			equipmentDischargeParameterService.save(equipmentDischargeParameter);
